...
public class Networking extends AsyncTask<Object, Integer, Object> {
    ...
    private static final String SERVER_PATH = "https://api.yelp.com/v3/";
    private static final String API_KEY = "Bearer VHU5423PonyHZZhsWAJEpjSK1WWlTH7Jv8tAk18vDtcUWCG6qKbIH4llHum7tUdUE1Pyz2IKWmVkoRg4rzdiJsJJQYOuR_Jv-eAmKK2Y7HDGRN_0ccjBfv27Gd8_YnYx";
    ...
    private List<Item> searching(String term, double latitud, double longitud){
        ...
        try {
            String response = post(SERVER_PATH+"businesses/search?term="+term+"&latitude="+latitud+"&longitude="+longitud);
    ...
    private Item getById(String id){
        Item i = null;
        try {
            String response = post(SERVER_PATH+"businesses/"+id);
            Gson gson = new GsonBuilder().setPrettyPrinting().disableHtmlEscaping().serializeNulls().create();
    ...
    private ArrayList<String> autocompleteList(String term, double latitud, double longitud){
        ...
        try {
            String response = post(SERVER_PATH+"autocomplete?text="+term+"&latitude="+latitud+"&longitude="+longitud);
            Gson gson = new GsonBuilder().setPrettyPrinting().disableHtmlEscaping().serializeNulls().create();
            JSONObject jsonObject = new JSONObject( gson.fromJson(response, JsonObject.class).toString() );
    ...
    String post(String url) throws IOException, NullPointerException {
